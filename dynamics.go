...
type BaseObjType Clip // TODO: Find the correct type

type Clip struct {
	input InputType
	min, max, mul, add float64
	in_fader *Clip
}
...
func (c *Clip) SetInput(x InputType, fadetime float64) { // fadetime=0.05
	c.input = x
	(c.in_fader).SetInput(x, fadetime)
}

// Replace the "min" attribute.
func (c *Clip) SetMin(x float64) {
	c.min = x
	x, lmax := ConvertArgsToLists(x)
	for i, obj := range c.base_objs {
		c.base_objs[i] = obj.SetMin(wrap(x, i))
	}
}

// Replace the "max" attribute
func (c *Clip) SetMax(x float64) {
	c.max = x
	x, lmax := ConvertArgsToLists(x)
	for i, _ := range c.base_objs {
		c.base_objs[i] = obj.SetMax(wrap(x, i))
	}
}

// TODO: Convert the rest to Go
/*
